...
const { deleteOrderItem, getOrderItemById, getOrderItemsByOrder, getOrdersItems, insertOrderItem, updateOrderItem, bulkDeleteOrderItems } = require("../services/orderItems.service");
...
module.exports = {
      ...
      addToOrder: async (req, res) => {
            try {
                  ...
                  if (order) {
                        ...
                        const itemToAdd = itemsFromOrder?.find((item) => item.productId === productId);
                        ...
                        if (itemToAdd) {
                              ...
                              const updateOrderItemFetch = await updateOrderItem(orderItemData, itemToAdd.id);
      ...
      removeOneItemFromOrder: async (req, res) => {
            try {
                  ...
                  if (req.user.id == item.order.userId) {
                        if (item.quantity > 1) {
      ...
      clearOrder: async (req, res) => {
            try {
                  ...
                  if (!order) return res.status(400).json(`La orden ${orderId} no existe`);
                  if (req.user.id == order.userId) {
                        ...
                        return itemsDeleteResult && orderDeleteResult ? res.status(200).json("Orden eliminada correctamente") : res.status(400).json("Hubo un error al eliminar la orden");
                  }
                  res.status(400).json("Esta orden pertence a otro usuario");
            } catch (error) {
                  console.error(error);
